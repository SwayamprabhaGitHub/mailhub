...
const UiEmailTypeBody = ({ setNoOfMailOnCurrPage, noOfMailOnCurrPage }) => {
  ...
  const nextPage = () => {
    if(totalNumOfMails - noOfMailOnCurrPage > 20)
    setNoOfMailOnCurrPage((prevNoOfMailOnCurrPage) => prevNoOfMailOnCurrPage + 20);
  };
 
  const prevPage = () => {
    if(noOfMailOnCurrPage > 0)
    setNoOfMailOnCurrPage((prevNoOfMailOnCurrPage) => prevNoOfMailOnCurrPage - 20);
  };
    ...
    <>
      ...
      <div className="flex items-center justify-between px-4">
        <div className="flex items-center gap-2 text-gray-700 py-2">
          ...
          {selectedEmailsArray.length > 0 && selectedMailPath !== "trash" && <div onClick={handleTrashEmail} className="p-2 rounded-full hover:bg-teal-200/30 transition-all duration-500 ease-in-out ">
            <FaTrash size={"16px"} />
          </div>}
          {selectedEmailsArray.length > 0 && selectedMailPath === "trash" && <div onClick={handleRestoreEmail} className="p-1 rounded-md hover:text-teal-700 hover:font-semibold hover:bg-teal-200/30 transition-all duration-500 ease-in-out text-black">
            <div>Restore</div>
          </div>}
          {selectedEmailsArray.length > 0 && selectedMailPath === "trash" && <div onClick={handleDeleteEmail} className="p-1 rounded-md hover:text-rose-700 hover:font-semibold hover:bg-rose-200/30 transition-all duration-500 ease-in-out text-black">
