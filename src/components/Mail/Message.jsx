...
const Message = ({ email, index }) => {
  ...
  return (
    <div
      ...
      className={`flex items-center justify-between ${!email?.read && "bg-[#FBFBF9]" } border-b border-gray-200 px-4 text-sm cursor-pointer hover:bg-rose-300/20 hover:shadow-md hover:-translate-y-1 transition-all duration-300 ease-in-out animate-slideUp opacity-0 [animation-delay:var(--delay)]`}
    >
      ...
      <div className="flex-1 ml-4 flex items-center gap-6">
        <p className={`text-black ${!email?.read && 'font-bold'} w-64`}>{(selectedMailPath !== "sent" && email?.from) || (selectedMailPath === "sent" && email?.to)}</p>
